...
- **2024-06-05** – Audited legacy `.select-input` and `.input-group.has-error` rules to prep the next migration slice. Documented neutral and dark-mode backgrounds, 6px radii, and mint focus rings for Select along with matching option backgrounds. Captured the red border + helper visibility toggle used by error states so Input/Select/DateInput can share a single helper/error presenter. **Next:** update `Select` styles to reuse the shared field surface tokens, then standardise the helper/error stack across all form controls (with dark-mode parity) before moving to Button variant gaps.

### Near-term implementation notes

- **Select parity:** map the light surface (`rgb(235 235 235)`) and dark-mode surface (`rgba(23,123,154,0.2)`) to existing neutral/dark tokens, keep 6px radii, and reuse the mint focus ring already mirrored in Input/TextArea. Ensure `<option>` backgrounds match the control surface in both themes and that text colour follows the same neutral/dark pairing. Bring white/readonly variants into the shared field styles rather than bespoke Select rules.
- **Error/helper presenter:** the legacy `.input-group.has-error` flips helper visibility (`display: block`), applies a red border to `.input-field`, and swaps placeholder colour while keeping the mint ring on focus. Build a single helper/error component used by Input, Select, and DateInput that can toggle visibility, apply tokenised error borders/rings, and handle dark-mode overrides without duplicating logic per control.
- **Wave 1 completion gates:** treat Select parity + shared helper/error handling as the remaining blockers for Wave 1 form work. After landing these, move to Button variant parity (outline-white/modal, dark/dark-secondary, black/white, alert/edit/inactive) with a focus on tokenised surfaces and hover/active/focus states.
